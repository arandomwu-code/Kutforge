...
class VideoEditor extends preact.Component {
  ...
  hydrateMediaFromSaved = (savedMedia) => {
    const media = (savedMedia || []).map(m => ({ ...m, url: window.kutforgeAPI.pathToFileUrl(m.path) }));
    media.forEach(m => {
      if (m.type === 'image') {
        const img = new Image();
        img.src = m.url;
  ...
  drawFrame = (ctx, w, h, t) => {
    ...
    order.forEach(track => {
      ...
      const trackClips = this.state.clips.filter(c => c.trackId === track.id).sort((a, b) => a.start - b.start);
      ...
      try {
        if (active && t < clip.start + active.duration) {
          const prevMedia = this.state.media.find(m => m.id === active.prev.mediaId);
          if (prevMedia) {
  ...
  startExport = async () => {
    ...
    const outPath = await window.kutforgeAPI.chooseExportPath(this.state.projectName, this.state.exportFormat);
