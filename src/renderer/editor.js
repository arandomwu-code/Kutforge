...
class VideoEditor extends preact.Component {
  state = {
    ...
    exportFileName: '',
    exportFps: parseInt((this.props && this.props.defaultExportFps) || '60', 10),
    exportPhase: '',
    canUndo: false,
    canRedo: false,
    toast: null,
    ...
    // Custom title bar (Windows/Linux only) - see main.js's
    // IS_CUSTOM_TITLEBAR_PLATFORM and componentDidMount below.
  ...
  componentDidMount() {
    ...
    // The custom title bar (Windows/Linux - see main.js) needs to know both
    // whether it should render at all, and the window's current maximized
    // state, to show the right maximize-vs-restore icon and stay correct if
    // the window gets maximized/restored some other way (double-clicking
    // the title bar itself, a Windows snap gesture, the View menu, etc.),
    // not just via its own button.
    window.kutforgeAPI.isCustomTitlebar().then(v => this.setState({ isCustomTitlebar: v })).catch(() => {});
    window.kutforgeAPI.isWindowMaximized().then(v => this.setState({ isMaximized: v })).catch(() => {});
    window.kutforgeAPI.onWindowMaximizedChange(v => this.setState({ isMaximized: v }));
  }
  ...
  minimizeWindow = () => window.kutforgeAPI.minimizeWindow();
  toggleMaximizeWindow = () => window.kutforgeAPI.toggleMaximizeWindow();
  closeWindow = () => window.kutforgeAPI.closeWindow();
  popupMenu = (label) => window.kutforgeAPI.popupMenu(label);
  ...
  classifyMediaType = (p) => {
    ...
    // Keep this list in sync with VIDEO_EXT/AUDIO_EXT in main.js - that one
    // gates the "Import Media..." dialog, this one is what actually decides
    // how a file gets handled (and is the *only* gate for drag-and-drop,
    // which bypasses the dialog's filter entirely).
    if (['mp4', 'mov', 'm4v', 'webm', 'mkv', 'avi', 'wmv', 'flv', 'f4v', 'mpg', 'mpeg', 'm2v', 'ts', 'mts', 'm2ts', '3gp', '3g2', 'ogv', 'vob', 'asf', 'mxf', 'divx'].includes(ext)) return 'video';
    if (['mp3', 'wav', 'aac', 'm4a', 'flac', 'ogg', 'wma', 'opus', 'aiff', 'aif', 'amr'].includes(ext)) return 'audio';
    if (['png', 'jpg', 'jpeg', 'gif', 'webp', 'bmp'].includes(ext)) return 'image';
    return null;
  };

  // Media is referenced by its path on disk - the file is never copied.
  // Duration/width/height come from the bundled ffprobe (via
  // window.kutforgeAPI.probeMediaFile), not from the HTML5 <video>/<audio>
  // element - ffprobe understands essentially everything the export
  // pipeline can already decode, while Chromium's own media element only
  // recognizes a much narrower set of formats. That element is still used
  // for the sidebar thumbnail (best-effort only - a file ffprobe reads fine
  // but Chromium can't decode still imports correctly, just without a
  // preview thumbnail; the sidebar already shows a placeholder for that).
  buildMediaItem = (filePath) => new Promise((resolve, reject) => {
    ...
    if (type === 'video') {
      window.kutforgeAPI.probeMediaFile(filePath).then((info) => {
        const duration = info.duration > 0 ? info.duration : 5;
        const width = info.width > 0 ? info.width : 1920;
        const height = info.height > 0 ? info.height : 1080;
        let done = false;
        const finish = (thumb) => { if (done) return; done = true; resolve({ id, name, type: 'video', path: filePath, url, duration, width, height, thumb }); };
        // A file that ffprobe can read but Chromium's <video> can't decode
        // at all (onerror, or nothing ever fires) should still import
        // successfully - metadata already came from ffprobe above, so the
        // only thing at stake here is the thumbnail, which just stays
        // empty in that case rather than blocking the import.
        const safetyTimer = setTimeout(() => finish(null), 4000);
        ...
        const grab = () => {
          ...
          try {
            ...
            finish(cvs.toDataURL('image/jpeg', 0.7));
          } catch (e) { finish(null); }
        };
        v.onloadedmetadata = () => {
          try { v.currentTime = Math.min(1, (v.duration || duration) / 2); v.onseeked = grab; } catch (e) { grab(); }
          fallbackTimer = setTimeout(grab, 500);
        };
        v.onerror = () => { clearTimeout(safetyTimer); finish(null); };
      }).catch((e) => reject(e));
    } else if (type === 'audio') {
      window.kutforgeAPI.probeMediaFile(filePath).then((info) => {
        const duration = info.duration > 0 ? info.duration : 5;
        resolve({ id, name, type: 'audio', path: filePath, url, duration, width: 0, height: 0, thumb: null });
      }).catch((e) => reject(e));
    } else if (type === 'image') {
      const img = new Image();
  ...
  // Media import (addFiles/buildMediaItem above) has never gone through
  // pushHistory/undo - it's not clip/track state, it's the separate list of
  // files the project knows about. Removal mirrors that on purpose, rather
  // than calling pushHistory here: pushHistory only ever snapshots
  // {tracks, clips}, never media, so undoing this would put the clips this
  // removes back on the timeline while leaving the media itself gone -
  // clips quietly pointing at nothing. The confirm() below is the safety
  // net instead of undo, same as newProject/removeTrack use it for their
  // own not-undoable actions.
  removeMedia = (media) => {
    const usingClips = this.state.clips.filter(c => c.mediaId === media.id);
    const msg = usingClips.length > 0
      ? `Remove "${media.name}"? This also removes ${usingClips.length} clip${usingClips.length === 1 ? '' : 's'} using it from the timeline. This can\u2019t be undone.`
      : `Remove "${media.name}" from this project? This can\u2019t be undone.`;
    if (!confirm(msg)) return;
    delete this.imageEls[media.id];
    this.setState(s => {
      const removedClipIds = new Set(s.clips.filter(c => c.mediaId === media.id).map(c => c.id));
      return {
        media: s.media.filter(m => m.id !== media.id),
        clips: s.clips.filter(c => c.mediaId !== media.id),
  ...
  // ---------- clip editing ----------
  // During a clip 'move' drag, figures out which track lane (if any) the
  // pointer is currently over - restricted to `allowedType`, since a clip
  // is never allowed to land on a track of the other type (a video/image
  // clip only moves between video tracks, an audio clip only between audio
  // tracks - the same video/audio split every other part of the app
  // already enforces, e.g. handleDropOnTrack above). Returns null if the
  // pointer isn't over any lane of that type (off the end of the list,
  // over the ruler, between tracks, hovering a wrong-type track, etc.) -
  // callers fall back to whatever track was already in play, so a clip
  // never gets orphaned mid-drag by a moment of being off any valid lane.
  trackIdAtY = (clientY, allowedType) => {
    for (const t of this.state.tracks) {
      if (t.type !== allowedType) continue;
      const el = this.trackLaneEls[t.id];
      if (!el) continue;
      const rect = el.getBoundingClientRect();
      if (clientY >= rect.top && clientY < rect.bottom) return t.id;
    }
    return null;
  };

  // Free-space bounds on `trackId` around `desiredStart` - the gap that
  // contains it if it's inside one, or the nearest edges if `desiredStart`
  // actually overlaps an existing clip there. Takes the desired position
  // explicitly (recomputed fresh on every drag event) rather than reading
  // `clip`'s own start/duration, because - unlike handleClipMouseDown's
  // bounds below, which describe a fixed envelope the clip can slide
  // within on the track it already lives on, since its same-track
  // neighbors don't change mid-drag - there's no such fixed envelope on a
  // track the clip doesn't actually live on. The relevant gap there can
  // only be found relative to wherever it's currently being dragged
  // toward, which moves every event.
  computeTrackBounds = (clip, trackId, desiredStart) => {
    const siblings = this.state.clips.filter(c => c.trackId === trackId && c.id !== clip.id).sort((a, b) => a.start - b.start);
    let prevEnd = 0;
    let nextStart = null;
    for (const c of siblings) {
      if (c.start + c.duration <= desiredStart + 0.001) {
  ...
  updateDragFromEvent = (e) => {
    ...
    let updated = { ...clip };
    if (mode === 'move') {
      // Which track the pointer is over right now, if any valid (same
      // type) one - falling back to the clip's own starting track covers
      // both "not currently over any lane" and "back over the original
      // track", which need the same treatment anyway.
      const originalTrack = this.state.tracks.find(t => t.id === clip.trackId);
      const hoveredTrackId = originalTrack ? this.trackIdAtY(e.clientY, originalTrack.type) : null;
      const targetTrackId = hoveredTrackId || clip.trackId;
      // On the original track, reuse the bounds computed at drag-start
      // (which already exclude its transition partner as a collision
      // boundary, see handleClipMouseDown) - anywhere else, compute fresh
      // bounds for that track. Recomputed from `clip` (the drag's
      // original snapshot) + `dt` on every single move event, same as the
      // rest of this branch already does - not built up incrementally -
      // so waffling back and forth across a track boundary mid-drag can't
      // accumulate any stale state.
      const bounds = targetTrackId === clip.trackId ? { prevEnd, nextStart } : this.computeTrackBounds(clip, targetTrackId, clip.start + dt);
      const lower = Math.max(0, bounds.prevEnd);
      const upper = bounds.nextStart == null ? Infinity : bounds.nextStart - clip.duration;
      updated.start = Math.max(lower, Math.min(clip.start + dt, upper));
      updated.trackId = targetTrackId;
      // A transition is a same-track, sequential relationship with
      // whichever clip precedes it - actually landing on a different
      // track severs that (export independently re-validates adjacency
      // either way, so this isn't a correctness fix, just keeps the
      // clip's own data - and the side panel showing it - from claiming a
      // transition that no longer makes positional sense). Ending up back
      // on the original track by release time restores it, same as if
      // the clip had never left.
      updated.transitionIn = targetTrackId === clip.trackId ? clip.transitionIn : { type: 'none', duration: 0, fromClipId: null };
  ...
  buildEdl = () => {
    const s = this.state;
    const defaultFilters = { brightness: 100, contrast: 100, saturation: 100, blur: 0, grayscale: 0, sepia: 0, vignette: 0 };
    // Only media actually placed on the timeline needs to exist for export
    // to succeed - runExport already only ever looks up media by walking
    // edl.clips (see ffmpegExport.js), never the full edl.media list, so an
    // unused sidebar import doesn't need to be in here at all. That matters
    // because there's currently no way to remove an import from the sidebar
    // once added (see kutforge-update-34-review.md) - without this filter,
    // any file that ever got imported and later moved, renamed, or deleted
    // would permanently fail validateEdl's existence check and block
    // exporting the project, even though nothing on the timeline still
    // referenced it.
    const usedMediaIds = new Set(s.clips.map(c => c.mediaId));
    return {
      aspect: s.aspect,
      ...
      media: s.media.filter(m => usedMediaIds.has(m.id)).map(m => ({ id: m.id, type: m.type, path: m.path, duration: m.duration, width: m.width, height: m.height })),
    };
  };
  ...
  renderVals() {
    ...
    const media = s.media.map(m => ({
      ...
      // mousedown (not click) + stopPropagation/preventDefault, same
      // pattern as a clip's mute-icon-on-a-draggable-clip in the timeline
      // (see onToggleMuteMouseDown below) - stops the card's own
      // onClick (add to timeline) and native drag-start from ever firing
      // when the person is actually clicking this button, not the card.
      onRemoveMouseDown: (e) => { e.stopPropagation(); e.preventDefault(); this.removeMedia(m); },
    ...
    const fpsOptions = [30, 60, 120].map(f => ({
  ...
  renderHeader(vm) {
    return html`
      <div style="flex:none;display:flex;align-items:center;gap:8px;padding:12px 22px;background:var(--panel2);border-bottom:1px solid rgba(var(--ink-rgb),0.14);">
        ${this.state.isCustomTitlebar ? null : html`<div style="font-family:'Caprasimo',system-ui,sans-serif;font-size:19px;color:var(--brand);">Kutforge</div>`}
  ...
  renderSidebar(vm) {
    return html`
      <div onDrop=${vm.handleSidebarDrop} onDragOver=${vm.handleSidebarDragOver} style="width:250px;flex:none;background:var(--panel2);border-right:1px solid rgba(var(--ink-rgb),0.14);padding:14px;overflow-y:auto;display:flex;flex-direction:column;gap:10px;">
        ...
        ${vm.media.map(m => html`
          <div key=${m.id} draggable="true" onDragStart=${m.onDragStart} onClick=${m.onClick} title="Click to add to timeline, or drag onto a track" style="position:relative;border-radius:14px;background:var(--panel);padding:8px;cursor:grab;box-shadow:0 1px 2px rgba(46,43,37,0.14);">
            <button onMouseDown=${m.onRemoveMouseDown} draggable="false" title="Remove from project" style="position:absolute;top:14px;right:14px;z-index:2;width:22px;height:22px;padding:0;border:none;border-radius:999px;background:rgba(46,43,37,0.6);color:#f5ead8;display:flex;align-items:center;justify-content:center;cursor:pointer;">
              <svg viewBox="0 0 24 24" width="12" height="12" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><path d="M18 6L6 18"></path><path d="M6 6l12 12"></path></svg>
            </button>
  ...
  // ---------- custom title bar ----------
  // Windows/Linux only (main.js's IS_CUSTOM_TITLEBAR_PLATFORM) - stands in
  // for the native title bar + menu bar that frame:false removes there
  // entirely. Rendered unconditionally by render() below, even before the
  // disclaimer gate is agreed to - without that, a frameless window with
  // nothing else drawn yet would have no way to move, minimize, or close
  // itself at all.
  //
  // Uses the exact same CSS custom properties as the rest of the app
  // (--panel2/--brand/--ink-rgb, set on the themed root container by
  // renderVals()'s themeVarsStyle - see render() below) so it's the same
  // color as the header row directly beneath it in every theme, not a
  // separately-colored strip bolted on top. Every var() reference carries
  // its own fallback (matching the dark theme's actual values) for the one
  // case those custom properties genuinely aren't in scope yet: rendered
  // pre-disclaimer, before any theme/project data has loaded, where the
  // fallback quietly takes over instead of the color references resolving
  // to nothing.
  renderTitleBar() {
    const s = this.state;
    const ink = 'var(--ink-rgb, 245,234,216)';
    const iconColor = `rgba(${ink},0.75)`;
    return html`
      <div onDblClick=${this.toggleMaximizeWindow} style="flex:none;height:34px;display:flex;align-items:center;background:var(--panel2, #171513);border-bottom:1px solid rgba(${ink},0.14);-webkit-app-region:drag;user-select:none;">
        <img src="titlebar-icon.png" width="17" height="17" style="margin-left:10px;border-radius:4px;flex:none;-webkit-app-region:drag;" />
        <div style="font-family:'Caprasimo',system-ui,sans-serif;font-size:12.5px;color:var(--brand, #e2985f);margin-left:8px;flex:none;-webkit-app-region:drag;">Kutforge</div>
        <div style="display:flex;align-items:center;height:100%;margin-left:14px;-webkit-app-region:no-drag;">
          ${['File', 'Edit', 'View', 'Help'].map(label => html`
            <button key=${label} onClick=${() => this.popupMenu(label)} class="titlebar-menu-btn" style="height:100%;padding:0 10px;border:none;background:transparent;color:${iconColor};font-size:12.5px;cursor:pointer;">${label}</button>
          `)}
        </div>
        ...
        <div style="display:flex;align-items:center;height:100%;-webkit-app-region:no-drag;">
          <button onClick=${this.minimizeWindow} title="Minimize" class="titlebar-win-btn" style="width:42px;height:100%;border:none;background:transparent;color:${iconColor};display:flex;align-items:center;justify-content:center;cursor:pointer;">
            <svg viewBox="0 0 10 10" width="10" height="10"><rect x="0" y="4.5" width="10" height="1" fill="currentColor"></rect></svg>
          </button>
          <button onClick=${this.toggleMaximizeWindow} title=${s.isMaximized ? 'Restore' : 'Maximize'} class="titlebar-win-btn" style="width:42px;height:100%;border:none;background:transparent;color:${iconColor};display:flex;align-items:center;justify-content:center;cursor:pointer;">
            ${s.isMaximized
              ? html`<svg viewBox="0 0 10 10" width="10" height="10" fill="none" stroke="currentColor" stroke-width="1"><path d="M2.5 0.5H9.5V7.5H7.5"></path><rect x="0.5" y="2.5" width="7" height="7"></rect></svg>`
              : html`<svg viewBox="0 0 10 10" width="10" height="10" fill="none" stroke="currentColor" stroke-width="1"><rect x="0.5" y="0.5" width="9" height="9"></rect></svg>`}
          </button>
          <button onClick=${this.closeWindow} title="Close" class="titlebar-win-btn titlebar-close-btn" style="width:42px;height:100%;border:none;background:transparent;color:${iconColor};display:flex;align-items:center;justify-content:center;cursor:pointer;">
            <svg viewBox="0 0 10 10" width="10" height="10" fill="none" stroke="currentColor" stroke-width="1.2" stroke-linecap="round"><path d="M0.5 0.5L9.5 9.5"></path><path d="M9.5 0.5L0.5 9.5"></path></svg>
          </button>
        </div>
      </div>
    `;
  ...
  renderDisclaimerGate() {
    ...
    return html`
      <div style="flex:1;min-height:0;display:flex;align-items:center;justify-content:center;background:#1c1a16;padding:24px;">
  ...
  render() {
    // Rendered first, and unconditionally (even pre-disclaimer) on
    // platforms that need it - see renderTitleBar's own comment for why.
    const titleBar = this.state.isCustomTitlebar ? this.renderTitleBar() : null;
    if (!this.state.disclaimerAgreed) {
      return html`
        <div style="position:fixed;inset:0;display:flex;flex-direction:column;background:#1c1a16;">
          ${titleBar}
          ${this.renderDisclaimerGate()}
        </div>
      `;
    }
...
preact.render(preact.h(VideoEditor, { defaultAspect: '16:9', defaultExportFps: '60' }), document.getElementById('app'));
