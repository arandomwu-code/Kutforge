...
class VideoEditor extends preact.Component {
  ...
  componentDidMount() {
    ...
    window.reelAPI.onMenuAction(this.handleMenuAction);
    window.reelAPI.onExportProgress(this.handleExportProgress);
    this.drawOnce();
  }
  ...
  handleGlobalClick = () => { if (this.state.contextMenu || this.state.showThemeMenu || this.state.showAdjustMenu) this.setState({ contextMenu: null, showThemeMenu: false, showAdjustMenu: false }); };
  ...
  loadSettings = async () => {
    try {
      const settings = await window.reelAPI.loadSettings();
      this.setState({ defaultTheme: (settings && settings.defaultTheme) || 'cream' });
    } catch (e) { /* keep the built-in default */ }
  };

  setDefaultTheme = (key) => {
    this.setState({ defaultTheme: key });
    window.reelAPI.saveSettings({ defaultTheme: key }).catch(() => {});
  ...
  // but let the person pick where the .reelproj.json file lives.
  hydrateMediaFromSaved = (savedMedia) => {
    const media = (savedMedia || []).map(m => ({ ...m, url: window.reelAPI.pathToFileUrl(m.path) }));
    media.forEach(m => {
      if (m.type === 'image') {
        const img = new Image();
        img.src = m.url;
  ...
  applyLoadedProject = (saved) => {
    ...
    const clips = (saved.clips || []).filter(c => validIds.has(c.mediaId));
    this.undoStack = []; this.redoStack = [];
    this.setState({
      projectName: saved.projectName || 'My Video',
      theme: saved.theme || this.state.defaultTheme,
      aspect: saved.aspect || this.state.aspect,
      tracks: saved.tracks || this.state.tracks,
      media, clips, selectedClipId: null, canUndo: false, canRedo: false,
    });
    this.seek(0);
  };
  ...
  loadProject = async () => {
    try {
      const saved = await window.reelAPI.loadAutosavedProject();
  ...
  saveProjectDebounced = () => {
    ...
    this._saveTimer = setTimeout(() => {
      window.reelAPI.saveAutosavedProject(this.currentProjectSnapshot()).catch(() => {});
    }, 600);
  };
  ...
  openProjectFromDisk = async () => {
    const res = await window.reelAPI.openProjectDialog();
    if (!res) return;
  ...
  saveProjectAs = async () => {
    const savedPath = await window.reelAPI.saveProjectAsDialog(this.currentProjectSnapshot());
  ...
  triggerFileInput = async () => {
    const paths = await window.reelAPI.openMediaDialog();
    if (paths && paths.length) this.addFiles(paths);
  };
  handleSidebarDrop = (e) => {
    e.preventDefault();
    const files = Array.from(e.dataTransfer.files || []);
    const paths = files.map(f => window.reelAPI.getPathForFile(f)).filter(Boolean);
  ...
  buildMediaItem = (filePath) => new Promise((resolve, reject) => {
    ...
    const url = window.reelAPI.pathToFileUrl(filePath);
  ...
  appendMediaAuto = (media) => { const trackId = media.type === 'audio' ? 'a1' : 'v1'; this.appendMediaToTrack(media, trackId); };

  // Fields every clip carries beyond the basics (start/duration/inPoint/volume/muted):
  // color filters (neutral = no visible change) and fade in/out durations
  // (0 = no fade). Kept as a small helper so every place a clip gets
  // created stays consistent.
  defaultClipExtras = () => ({ filters: { brightness: 100, contrast: 100, saturation: 100 }, fadeIn: 0, fadeOut: 0 });
  ...
  handleClipMouseDown = (e, clip, mode) => {
    ...
    let prevEnd = 0;
    siblings.forEach(c => { const end = c.start + c.duration; if (end <= clip.start + 0.001 && end > prevEnd) prevEnd = end; });
    let nextStart = null;
    siblings.forEach(c => { if (c.start >= clip.start + clip.duration - 0.001) { if (nextStart === null || c.start < nextStart) nextStart = c.start; } });
  ...
  handleFilterChange = (key, value) => {
    ...
    this.setState({
      clips: this.state.clips.map(c => c.id === this.state.selectedClipId
        ? { ...c, filters: { ...(c.filters || { brightness: 100, contrast: 100, saturation: 100 }), [key]: value } }
        : c),
    });
  };
  ...
  resetFilters = () => {
    if (!this.state.selectedClipId) return;
    this.pushHistory();
    this.setState({ clips: this.state.clips.map(c => c.id === this.state.selectedClipId ? { ...c, filters: { brightness: 100, contrast: 100, saturation: 100 } } : c) });
  };

  handleFadeChange = (key, seconds) => {
    if (!this.state.selectedClipId) return;
    this.setState({
      clips: this.state.clips.map(c => c.id === this.state.selectedClipId
        ? this.clampClipFades({ ...c, [key]: Math.max(0, seconds) })
        : c),
    });
  };
  ...
  splitAtPlayhead = () => {
    ...
    this.state.clips.forEach(c => {
      ...
      if (inRange && targeted) {
        ...
        newClips.push(this.clampClipFades({ ...c, id: this.uid(), start: t, duration: rightDur, inPoint: c.inPoint + leftDur, fadeIn: 0 }));
        did = true;
      } else newClips.push(c);
    });
    if (did) this.setState({ clips: newClips });
  ...
  deleteSelected = (ripple) => {
    ...
    if (ripple) clips = clips.map(c => (c.trackId === clip.trackId && c.start > clip.start) ? { ...c, start: c.start - clip.duration } : c);
  ...
  splitClipAt = (clipId) => {
    ...
    if (did) this.setState({ clips: newClips, contextMenu: null });
  ...
  duplicateClip = (clipId) => {
    ...
    const newClip = { ...clip, id: this.uid(), start };
  ...
  zoomOut = () => this.setState(s => ({ zoom: Math.max(6, s.zoom / 1.25) }));
  zoomToFit = () => {
  ...
  handleTimelineWheel = (e) => {
    ...
    this.setState(s => ({ zoom: Math.min(300, Math.max(fitZoom, s.zoom * factor)) }));
  };
  ...
  drawFrame = (ctx, w, h, t) => {
    ...
    order.forEach(track => {
      ...
      const clip = this.state.clips.find(c => c.trackId === track.id && t >= c.start - 0.0001 && t < c.start + c.duration + 0.0001);
      if (!clip) return;
      const media = this.state.media.find(m => m.id === clip.mediaId);
      if (!media) return;
      let src = null;
      ...
      try {
        ...
        ctx.filter = f ? `brightness(${f.brightness}%) contrast(${f.contrast}%) saturate(${f.saturation}%)` : 'none';
        ctx.drawImage(src, dx, dy, dw, dh);
        ctx.filter = 'none';
        const fadeAlpha = this.computeFadeAlpha(clip, t);
        if (fadeAlpha > 0) {
          // Only over this clip's own drawn rect, not the whole canvas -
          // otherwise a fading top-track clip narrower than the frame would
          // incorrectly wash over a bottom track peeking through its bars.
  ...
  syncMediaElements = (playing) => {
    this.state.clips.forEach(c => {
      ...
      if (!el) return;
      const track = this.state.tracks.find(tr => tr.id === c.trackId);
      el.muted = !!(c.muted || (track && track.muted));
      el.volume = c.volume == null ? 1 : c.volume;
      const active = this.currentTime >= c.start && this.currentTime < c.start + c.duration;
      if (active) {
  ...
  getExportDims = (res, aspect) => {
    const table = { '480p': [854, 480], '720p': [1280, 720], '1080p': [1920, 1080], '1440p': [2560, 1440], '4K': [3840, 2160] };
    const [w, h] = table[res] || table['1080p'];
    return aspect === '9:16' ? { w: h, h: w } : { w, h };
  };
  getBitrate = (res) => ({ '480p': 2500000, '720p': 5000000, '1080p': 8000000, '1440p': 16000000, '4K': 35000000 }[res] || 8000000);

  openExport = () => this.setState({ showExport: true, exportDone: false, exportError: null, exportRunning: false });
  ...
  buildEdl = () => {
    ...
    return {
      ...
      clips: s.clips.map(c => ({
        ...
        filters: c.filters || { brightness: 100, contrast: 100, saturation: 100 },
  ...
  startExport = async () => {
    ...
    const outPath = await window.reelAPI.chooseExportPath(this.state.projectName, this.state.exportFormat);
    if (!outPath) return; // person cancelled the save dialog
    ...
    try {
      ...
      await window.reelAPI.startExport(edl, outPath);
  ...
  cancelExport = () => { window.reelAPI.cancelExport(); };

  revealExportedFile = () => { if (this._exportOutPath) window.reelAPI.showItemInFolder(this._exportOutPath); };
  ...
  themes = {
    cream: { label: 'Default (Cream)', bg: '#f5ead8', panel: '#f9f4ed', panel2: '#ebddc5', text: '#201e1d', ink: '32,30,29', accent: '#c67139', accentText: '#f5ead8', brand: '#8c491a', laneAudio: '#f0fae1' },
    dark: { label: 'Dark', bg: '#201e1d', panel: '#2b2724', panel2: '#171513', text: '#f5ead8', ink: '245,234,216', accent: '#c67139', accentText: '#201e1d', brand: '#e2985f', laneAudio: '#2a3324' },
    sage: { label: 'Sage', bg: '#eef1e6', panel: '#f6f8f0', panel2: '#dfe6cd', text: '#201e1d', ink: '32,30,29', accent: '#7a8a5e', accentText: '#f5ead8', brand: '#5f6f45', laneAudio: '#e4ecd6' },
    blue: { label: 'Blue', bg: '#e9eef5', panel: '#f3f6fa', panel2: '#d7e2ee', text: '#201e1d', ink: '32,30,29', accent: '#4a6fa5', accentText: '#f5ead8', brand: '#33547a', laneAudio: '#dbe6f2' },
    red: { label: 'Red', bg: '#f5e9e6', panel: '#faf1ef', panel2: '#ecd5d0', text: '#201e1d', ink: '32,30,29', accent: '#b5473a', accentText: '#f5ead8', brand: '#7a2e24', laneAudio: '#f0d9d4' },
    purple: { label: 'Purple', bg: '#eee8f2', panel: '#f6f2f8', panel2: '#ddd0e6', text: '#201e1d', ink: '32,30,29', accent: '#7a5a9e', accentText: '#f5ead8', brand: '#513a6b', laneAudio: '#e6dbee' },
  };
  ...
  renderVals() {
    ...
    if (selectedClipData) {
      ...
      const filt = selectedClipData.filters || { brightness: 100, contrast: 100, saturation: 100 };
      ...
      selectedClip = {
        ...
        fadeMax: Math.max(0.1, Math.min(5, selectedClipData.duration / 2)),
        fadeInLabel: fadeIn > 0 ? `Fade in \u2014 ${fadeIn.toFixed(2)}s` : 'Fade in \u2014 off',
        fadeOutLabel: fadeOut > 0 ? `Fade out \u2014 ${fadeOut.toFixed(2)}s` : 'Fade out \u2014 off',
        onFadeInChange: (e) => this.handleFadeChange('fadeIn', parseFloat(e.target.value)),
        onFadeOutChange: (e) => this.handleFadeChange('fadeOut', parseFloat(e.target.value)),
      };
    }

    const HANDLE_W = 9;
    const tracksVM = s.tracks.map(track => {
      const clipsRaw = s.clips.filter(c => c.trackId === track.id).sort((a, b) => a.start - b.start);
      ...
      const clips = clipsRaw.map(c => {
        ...
        const bg2 = isAudioTrack ? (selected ? '#ccdbb2' : '#e1eecc') : (selected ? '#ffc6a5' : '#ffe1d0');
        const borderColor = isAudioTrack ? '#728157' : '#b2622d';
        return {
          id: c.id, left, width,
          ...
          textColor: isAudioTrack ? '#3d472b' : '#643312',
      ...
      return {
        ...
        laneBg: isAudioTrack ? t.laneAudio : t.panel,
    ...
    const timelineWidth = Math.max(900, total * s.zoom + 300);
    ...
    const resolutionOptions = ['480p', '720p', '1080p', '1440p', '4K'].map(r => ({
    ...
    const formatOptions = [{ key: 'webm', label: 'WebM' }, { key: 'mp4', label: 'MP4' }].map(o => ({
    ...
    const shortcuts = [
      ...
      { label: 'Colors, fades (select a clip first)', key: 'Adjust button' },
      { label: 'Toggle this panel', key: '?' },
    ];

    return {
      projectName: s.projectName, handleNameChange: this.handleNameChange,
      ...
      noSelection: !s.selectedClipId, hasSelection: !!s.selectedClipId, selectedClip,
      ...
      durationLabel: this.fmtTime(total), sizeLabel, overLimit,
      ...
      showHelp: s.showHelp, closeHelp: this.closeHelp, shortcuts,
  ...
  renderHeader(vm) {
    return html`
      <div style="flex:none;display:flex;align-items:center;gap:14px;padding:10px 16px;background:var(--panel2);border-bottom:1px solid rgba(var(--ink-rgb),0.14);">
        <div style="font-family:'Caprasimo',system-ui,sans-serif;font-size:19px;color:var(--brand);">Reel</div>
        <input value=${vm.projectName} onChange=${vm.handleNameChange} style="background:transparent;border:none;font-family:'Caprasimo',system-ui,sans-serif;font-size:15px;color:var(--text);outline:none;max-width:220px;padding:6px 8px;border-radius:8px;" />
  ...
  renderStage(vm) {
    return html`
      <div style="flex:1;display:flex;flex-direction:column;min-width:0;">
        <div style="flex:1;display:flex;align-items:center;justify-content:center;background:#2e2b25;position:relative;min-height:0;">
  ...
  renderTimelinePanel(vm) {
    return html`
      <div style="flex:none;height:262px;display:flex;flex-direction:column;background:var(--panel);border-top:1px solid rgba(var(--ink-rgb),0.14);">
        <div style="flex:none;height:44px;display:flex;align-items:center;gap:8px;padding:0 12px;border-bottom:1px solid rgba(var(--ink-rgb),0.1);">
          ...
          ${vm.hasSelection ? html`
            <div style="display:flex;align-items:center;gap:8px;padding-left:8px;border-left:1px solid rgba(var(--ink-rgb),0.14);margin-left:2px;">
              ...
              ${vm.selectedClip.isVisual ? html`
                <div style="position:relative;">
                  <button onClick=${vm.selectedClip.toggleAdjustMenu} title="Color adjustments and fades" style="display:flex;align-items:center;gap:5px;height:26px;padding:0 10px;border-radius:999px;border:1px solid rgba(var(--ink-rgb),0.16);background:${vm.selectedClip.filtersActive || vm.selectedClip.fadeInSeconds > 0 || vm.selectedClip.fadeOutSeconds > 0 ? 'rgba(198,113,57,0.14)' : 'transparent'};cursor:pointer;color:var(--text);font-size:11px;font-weight:600;">
                    <svg viewBox="0 0 24 24" width="13" height="13" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="13.5" cy="6.5" r=".5" fill="currentColor"></circle><circle cx="17.5" cy="10.5" r=".5" fill="currentColor"></circle><circle cx="8.5" cy="7.5" r=".5" fill="currentColor"></circle><circle cx="6.5" cy="12.5" r=".5" fill="currentColor"></circle><path d="M12 2C6.5 2 2 6.5 2 12s4.5 10 10 10c.926 0 1.648-.746 1.648-1.688 0-.437-.18-.835-.437-1.125-.29-.289-.438-.652-.438-1.125a1.64 1.64 0 0 1 1.668-1.668h1.996c3.051 0 5.555-2.503 5.555-5.554C21.965 6.012 17.461 2 12 2z"></path></svg>
                    Adjust
                  </button>
                  ${vm.selectedClip.showAdjustMenu ? html`
                    <div onMouseDown=${vm.stopPropagation} onClick=${vm.stopPropagation} style="position:absolute;bottom:calc(100% + 8px);left:0;background:var(--panel);border-radius:14px;box-shadow:0 8px 24px rgba(46,43,37,0.28);padding:14px;width:220px;z-index:60;display:flex;flex-direction:column;gap:10px;">
                      <div>
                        <div style="display:flex;justify-content:space-between;font-size:11px;color:rgba(var(--ink-rgb),0.6);margin-bottom:3px;"><span>Brightness</span><span>${Math.round(vm.selectedClip.brightness)}%</span></div>
                        <input type="range" min="0" max="200" value=${vm.selectedClip.brightness} onInput=${vm.selectedClip.onBrightnessChange} style="width:100%;" />
                      </div>
                      <div>
                        <div style="display:flex;justify-content:space-between;font-size:11px;color:rgba(var(--ink-rgb),0.6);margin-bottom:3px;"><span>Contrast</span><span>${Math.round(vm.selectedClip.contrast)}%</span></div>
                        <input type="range" min="0" max="200" value=${vm.selectedClip.contrast} onInput=${vm.selectedClip.onContrastChange} style="width:100%;" />
                      </div>
                      <div>
                        <div style="display:flex;justify-content:space-between;font-size:11px;color:rgba(var(--ink-rgb),0.6);margin-bottom:3px;"><span>Saturation</span><span>${Math.round(vm.selectedClip.saturation)}%</span></div>
                        <input type="range" min="0" max="200" value=${vm.selectedClip.saturation} onInput=${vm.selectedClip.onSaturationChange} style="width:100%;" />
                      </div>
                      <button onClick=${vm.selectedClip.resetFilters} disabled=${!vm.selectedClip.filtersActive} style="align-self:flex-start;padding:4px 10px;font-size:11px;border-radius:999px;border:1px solid rgba(var(--ink-rgb),0.16);background:transparent;cursor:pointer;color:var(--text);">Reset colors</button>
                      <div style="height:1px;background:rgba(var(--ink-rgb),0.12);"></div>
                      <div>
                        <div style="display:flex;justify-content:space-between;font-size:11px;color:rgba(var(--ink-rgb),0.6);margin-bottom:3px;"><span>${vm.selectedClip.fadeInLabel}</span></div>
                        <input type="range" min="0" max=${vm.selectedClip.fadeMax} step="0.05" value=${vm.selectedClip.fadeInSeconds} onInput=${vm.selectedClip.onFadeInChange} style="width:100%;" />
                      </div>
                      <div>
                        <div style="display:flex;justify-content:space-between;font-size:11px;color:rgba(var(--ink-rgb),0.6);margin-bottom:3px;"><span>${vm.selectedClip.fadeOutLabel}</span></div>
                        <input type="range" min="0" max=${vm.selectedClip.fadeMax} step="0.05" value=${vm.selectedClip.fadeOutSeconds} onInput=${vm.selectedClip.onFadeOutChange} style="width:100%;" />
                      </div>
                    </div>
                  ` : null}
                </div>
        ...
        <div style="flex:1;display:flex;min-height:0;">
          <div style="width:104px;flex:none;display:flex;flex-direction:column;background:var(--panel2);">
            <div style="height:26px;flex:none;"></div>
            ${vm.tracksVM.map(track => html`
              <div key=${track.id} style="height:60px;flex:none;display:flex;align-items:center;justify-content:space-between;padding:0 10px;border-bottom:1px solid rgba(var(--ink-rgb),0.12);border-top:1px solid rgba(var(--ink-rgb),0.12);">
                <span style="font-size:11px;font-weight:600;">${track.name}</span>
                <div style="display:flex;gap:3px;">
                  ${track.hasClips ? html`<button onClick=${track.onCloseGaps} title="Close gaps on this track" style="width:20px;height:20px;border-radius:6px;border:none;background:transparent;cursor:pointer;color:rgba(var(--ink-rgb),0.55);font-size:13px;line-height:1;">${'\u00bb'}</button>` : null}
                  <button onClick=${track.onToggleTrackMute} title="Mute track" style="width:20px;height:20px;border-radius:6px;border:none;background:transparent;cursor:pointer;color:${track.muteIconColor};">
          ...
          <div ref=${vm.setTimelineScroll} style="flex:1;overflow-x:auto;overflow-y:hidden;position:relative;">
            <div ref=${vm.setTimelineContent} style="position:relative;width:${vm.timelineWidth}px;height:100%;">
              <div onMouseDown=${vm.handleRulerMouseDown} style="height:26px;position:relative;background:var(--panel2);border-bottom:1px solid rgba(var(--ink-rgb),0.12);cursor:pointer;">
        ...
        <div onMouseDown=${vm.stopPropagation} style="width:min(460px,100%);display:flex;flex-direction:column;gap:14px;padding:26px;border-radius:28px;background:var(--panel2);box-shadow:0 12px 32px rgba(46,43,37,0.22);">
          ...
          ${vm.showExportForm ? html`
            ...
            <div style="display:flex;justify-content:flex-end;gap:8px;margin-top:4px;">
              <button onClick=${vm.closeExportDialog} style="padding:9px 18px;font-size:13px;border-radius:999px;border:1px solid rgba(var(--ink-rgb),0.16);background:transparent;cursor:pointer;">Cancel</button>
          ...
          ${vm.exportRunning ? html`
            ...
            <div style="display:flex;justify-content:flex-end;">
              <button onClick=${vm.cancelExport} style="padding:9px 18px;font-size:13px;border-radius:999px;border:1px solid rgba(var(--ink-rgb),0.16);background:transparent;cursor:pointer;">Cancel export</button>
            </div>
          ` : null}
          ...
          ${vm.exportDone ? html`
            ...
            <div style="display:flex;justify-content:flex-end;gap:8px;">
              <button onClick=${vm.revealExportedFile} style="padding:9px 18px;font-size:13px;border-radius:999px;border:1px solid rgba(var(--ink-rgb),0.16);background:transparent;cursor:pointer;">Show in folder</button>
